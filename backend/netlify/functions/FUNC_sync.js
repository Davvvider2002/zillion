...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Update devices table with the holder_hash for this device
    // This links device_hash → coin holder_hash for admin balance queries
    if (body.device_id && body.holder_hash) {
      const db = require('../../lib/supabase').getServiceClient();
      await db.from('devices')
        .update({ holder_hash: body.holder_hash, last_sync: new Date().toISOString() })
        .eq('device_hash', body.device_id)
        .catch(() => {}); // non-fatal
    }
