...
const CACHE_NAME    = 'zillion-merchant-v1.0.12';
...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(
    caches.keys().then(keys =>
      Promise.all(
        keys.filter(k => k.startsWith('zillion-merchant-') && k !== CACHE_NAME)
  ...
  event.respondWith(
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(response => {
        ...
        if (url.pathname.includes('merchant') || url.pathname === '/') {
          return caches.match('/merchant/') ||
                 caches.match('/merchant/index.html');
        }
      });
    })
  );
...
self.addEventListener('push', event => {
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Zillion', {
      body:    data.body    || '',
      icon:    '/merchant/icon-192.png',
      badge:   '/merchant/icon-192.png',
      vibrate: [200, 100, 200],
      data:    data,
